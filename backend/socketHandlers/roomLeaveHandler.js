  ...
  if (activeRoom) {
    ...
    if (isCreatorLeaving) {
      if (activeRoom.participants.length === 1) {
        ...
        console.log(`Room with ID ${roomId} has been closed as the creator left.`);
      } else {
        ...
        io.to(roomId).emit("room-updated", { participants: updatedParticipants }); // Notify participants
        console.log(
      ...
      if (updatedParticipants.length > 0) {
        ...
        io.to(roomId).emit("room-updated", { participants: updatedParticipants });
      } else {
